...
namespace phg {
    ...
    vector3d unproject(const vector3d &pixel, const phg::Calibration &calibration, const matrix34d &PtoWorld)
    {
        ...
        vector3d local_point; // TODO 102 пустите луч pixel из calibration а затем возьмите на нем точку у которой по оси +Z координата=depth
        local_point = calibration.unproject(vector2d(pixel[0], pixel[1])) * depth;
        ...
        for (ptrdiff_t j = 0; j < height; ++j) {
            for (ptrdiff_t i = 0; i < width; ++i) {
...

                {
                    ...
                    float d = (1 - (float)iter / (float)NITERATIONS) / 2;
                    ...
                    np = cv::normalize(n0 + randomNormalObservedFromCamera(cameras_RtoWorld[ref_cam], r) * d);
                    // Стало работать немного дольше, в результатах особых изменений не заметила
        ...
        for (int chessboard_pattern_step = 0; chessboard_pattern_step < 2; ++chessboard_pattern_step) {
            ...
            for (ptrdiff_t j = 0; j < height; ++j) {
                for (ptrdiff_t i = (j + chessboard_pattern_step) % 2; i < width; i += 2) {
                    ...
                    std::vector<size_t> best_cost_ind(hypos_depth.size());
                    for (int i = 0; i < best_cost_ind.size(); i++) {
                        best_cost_ind[i] = i;
                    }
                    std::sort(best_cost_ind.begin(), best_cost_ind.end(),
                              [&hypos_cost] (size_t i, size_t j) -> bool { return hypos_cost[i] > hypos_cost[j]; });
                    best_cost_ind.resize(std::min<size_t>(8, best_cost_ind.size()));
                    ...
                    for (size_t i = 0; i < best_cost_ind.size(); ++i) {
                        // эту гипотезу мы сейчас рассматриваем как очередного кандидата
                        size_t hi = best_cost_ind[i];
    ...
    float PMDepthMapsBuilder::estimateCost(ptrdiff_t i, ptrdiff_t j, double d, const vector3d &global_normal, size_t neighb_cam)
    {
        ...
        for (ptrdiff_t dj = -COST_PATCH_RADIUS; dj <= COST_PATCH_RADIUS; ++dj) {
            for (ptrdiff_t di = -COST_PATCH_RADIUS; di <= COST_PATCH_RADIUS; ++di) {
                ...
                intensity += cameras_imgs_grey[neighb_cam].at<unsigned char>(v+1, u+1) * dx * dy;
                intensity += cameras_imgs_grey[neighb_cam].at<unsigned char>(v, u+1) * dx * (1 - dy);
                intensity += cameras_imgs_grey[neighb_cam].at<unsigned char>(v+1, u) * (1 - dx) * dy;
                intensity += cameras_imgs_grey[neighb_cam].at<unsigned char>(v, u) * (1 - dx) * (1 - dy);

                patch1.push_back(intensity / 255.0f);
            }
        }
        ...
        for (size_t k = 0; k < n; ++k) {
            ...
            x += (patch0[k] - mean0) * (patch0[k] - mean0);
            y += (patch1[k] - mean1) * (patch1[k] - mean1);
        }
        if (std::abs(x * y) < 1e-3) {
            return NO_COST;
        }
    ...
    float PMDepthMapsBuilder::avgCost(std::vector<float> &costs)
    {
        ...
        // Можно попробовать посмотреть на соседей, не сильно ли большая разница с ними
