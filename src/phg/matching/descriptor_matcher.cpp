                                                    ...
                                                    std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (int i = 0; i < n_matches; i++) {
        ...
        if (matches[i].size() == 1 || (matches[i][1].distance != 0 && matches[i][0].distance / matches[i][1].distance < 0.6)) { // My Param, на лекции был 0.8, но у меня так тест один не проходит
            // Там вообще не должно быть == 0, но не хочется UB
        ...
        for (int j = 0; j < total_neighbours; j++) {
            if (distances2_query.at<float>(i, j) <= radius2_query) {
                for (int k = 0; k < total_neighbours; k++) {
                    if (indices_query.at<int>(i, j) == indices_train.at<int>(i, k)
                            && distances2_train.at<float>(i, k) <= radius2_train) {
                        n_filtered++;
                    }
                }
            }
        }
        if (n_filtered >= consistent_matches) {
